...
PFont BTR;
int PosXD1,PosXD2,PosXD3,PosXD4,PosXD5,PosXLogo,PosYLogo,TamXLogo,TamYLogo,PosXC,PosXJ,PosXK,PosXL,PosXG,PosYG,PosXRect,PosYRect,TamXRect,TamYRect,TamXG,TamYG,Opacidad,Frame,Caracter1,Caracter2,Caracter3,Fase,PosXText,PosXText2;
boolean Boton,Terminado,Integrante,Texto,TerminadoDiscos;
...
void mousePressed(){
  if(mouseX >= 270 && mouseX <= 370 && mouseY >= 335 && mouseY <= 385){
    Boton = true;
            ...
            /*textSize(38);
            text("Kendall\nSchmidt",PosXText2,240);
            image(Kendall,PosXK,125,304,385);   
            if(PosXText2 > 450 && Texto == false){
              PosXText2 = PosXText2 - 5;
            }else if(PosXText2 <= 450){
              Texto = true;
            }
            if(Texto == true){
              if(PosXText2 < 700){
